...
def get_var(variable, dates, years, return_store=False):
    ...
    if return_store:
        ...
        da = lz.streamline_and_normalise_zarr(variable, da, split_steps=[0,2,4,6,8])

        return da, list(np.unique(dates))

def open_mfzarr(file_names, use_modify=False, centre = [-1.25, 36.80], window_size = 2, months=[3,4,5,6]):
    #client = Client()
    #client.restart()
    def modify(ds, use_modify=False,centre = [-1.25, 36.80], window_size = 2, months=[3,4,5,6]):
    ...
    tasks = [dask.delayed(modify)(task,use_modify=use_modify,
                                 ...
                                 months=months) for task in open_tasks]
...
def get_all_ifs(years,centre = [-1.25, 36.80], window_size = 20, months=[3,4,5,6], ignore_truth=False, variables = None):
    ...
    for year in years:
        ...
        if ignore_truth:
            dates = dates = list(np.arange("%i-01-01"%year,"%i-01-01"%(year+1),np.timedelta64(1,"D"),dtype='datetime64[D]').astype("str"))
        else:
        ...
        ds, dates_modified = open_mfzarr(files, use_modify=True, centre = centre, 
                                         window_size = window_size, months=[3,4,5,6])
    ...
    ds_truth_and_mask =  lz.load_truth_and_mask(np.array(dates_all, dtype="datetime64[ns]").flatten(),
                                                       time_idx=[1,2,3,4])
    ds_constants = lz.load_hires_constants(batch_size=4)
    ...
    return (
            ...
            ds_truth_and_mask.rename({"latitude": "lat", "longitude": "lon"}).sel(\
                time=ds_truth_and_mask.time.dt.month.isin([3,4,5,6])).sel({'lat':slice(centre[0]-window_size,centre[0]+window_size ),
    ...
    if generator:
        ...
        for year in years:
            ...
            for var in all_fcst_fields.keys():
                ...
                if os.path.exists(FCST_PATH+str(year)+'/'+all_fcst_fields[var]+'.zarr'):
                    continue
                else:
                    print('Creating consolidated file for variable', var)
                    da, _ = get_var(var, dates, years)
                    da.to_zarr(FCST_PATH+str(year)+'/'+da['field'].values[0]+'.zarr')
                    da.close()
