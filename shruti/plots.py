...
def plot_sequences(
    ...
    for kk in range(num_cases):
        ...
        if cond.shape[-1] > 1:
            all_fcst_fields_list = [key for key in data.all_fcst_fields.keys()]
            tpidx = 4 * all_fcst_fields_list.index("Total Precipitation")+3
        else:
            tpidx = 0
        plot_img(data.denormalise(cond[0, :, :, tpidx]), value_range=value_range)
